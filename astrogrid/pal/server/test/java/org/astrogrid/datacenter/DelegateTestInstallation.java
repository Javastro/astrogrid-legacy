/*$Id: DelegateTestInstallation.java,v 1.1 2005/02/17 18:37:35 mch Exp $
 ...
 *
 */
public abstract class DelegateTestInstallation extends ServerTestCase {

    /**
     * Constructor for AbstractTestInstallation.
     * @param arg0
     */
    public DelegateTestInstallation(String arg0) {
        super(arg0);
    }
 ...
 */
    ...
    public void testDoBlockingQuery() throws Throwable {
        try {
        QuerySearcher del = createDelegate();
        FileProcessor fp = new FileProcessor() {
            protected void processStream(QuerySearcher del, InputStream is) throws Exception{
                doQuery(del,is);
            }
        };
        fp.findFiles(del);
    ...
    protected QuerySearcher createDelegate() {

        System.out.println("Connecting to datacenter service at " + serviceURL.toString());
        QuerySearcher del = null;
        try {
            del = DatacenterDelegateFactory.makeQuerySearcher(LoginAccount.ANONYMOUS, serviceURL.toString(), DatacenterDelegateFactory.ASTROGRID_WEB_SERVICE) ;// pity it can't take a URL
        } catch (IOException e) {
            e.printStackTrace();
    ...
    protected void doQuery(QuerySearcher del, InputStream is) throws Exception {
       fail("Test needs to be updated to ADQL 0.7.4+");
       /*
       assertNotNull(is);
       Select adql = Select.unmarshalSelect(new InputStreamReader(is));
       assertNotNull("query input is null",adql);
       InputStream result = del.askQuery(new AdqlQuery(ADQLUtils.toQueryBody(adql)), QuerySearcher.VOTABLE);
        assertNotNull("Result of query was null",result);
       Element vo = DomHelper.newDocument(result).getDocumentElement();
       assertNotNull(vo);
    ...
    protected void doNonBlockingQuery(QuerySearcher del, InputStream is) throws Exception {
       fail("Test needs to be updated to ADQL 0.7.4+");
       /*
        assertNotNull(is);
        Select adql = Select.unmarshalSelect(new InputStreamReader(is));
   ...
   protected abstract class FileProcessor {
    ...
    protected abstract void processStream(QuerySearcher del,InputStream is) throws Exception;
     ...
     * @throws Exception */
    public void findFiles(QuerySearcher del) throws Exception{
...
Revision 1.2.30.2  2004/11/29 21:52:18  mch
