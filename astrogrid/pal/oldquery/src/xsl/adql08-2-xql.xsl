...
<xsl:stylesheet xmlns="http://www.ivoa.net/xml/ADQL/v0.8" xmlns:ad="http://www.ivoa.net/xml/ADQL/v0.8" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0">
   ...
   <xsl:template match="ad:SelectionList">
	  ...
	  <xsl:apply-templates select="../ad:Where">
		<xsl:with-param name='selectPiece' select='$list'/>
	  </xsl:apply-templates>      
   </xsl:template>
   ...
   <xsl:template match="*[substring-after(@xsi:type, ':') = 'columnReferenceType'] | *[@xsi:type = 'columnReferenceType']">   
      ...
      <xsl:choose>
         <xsl:when test="@xpathName != ''">
			<xsl:text>$x/</xsl:text><xsl:value-of select="@xpathName" />   
         </xsl:when>
         <xsl:otherwise>
