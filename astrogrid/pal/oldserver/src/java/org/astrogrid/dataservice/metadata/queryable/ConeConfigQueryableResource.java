/*
 * $Id: ConeConfigQueryableResource.java,v 1.2 2007/06/08 13:16:11 clq2 Exp $
 *
   ...
   public SearchField[] getSpatialFields(SearchGroup parent) throws IOException
   {
      if (parent.getId().equals(ConfigFactory.getCommonConfig().getString(CONE_SEARCH_TABLE_KEY))) {
         ...
         SearchField[] fields = {
            ...
            TableMetaDocInterpreter.getColumnInfoByID(catalogIDs[0], parent.getId(), ConfigFactory.getCommonConfig().getString(CONE_SEARCH_RA_COL_KEY)),
            //getColumn(catalogNames[0], parent.getName(), ConfigFactory.getCommonConfig().getString(CONE_SEARCH_DEC_COL_KEY))
            TableMetaDocInterpreter.getColumnInfoByID(catalogIDs[0], parent.getId(), ConfigFactory.getCommonConfig().getString(CONE_SEARCH_DEC_COL_KEY))
         };
         return fields;
      }
      else {
