...
import org.astrogrid.store.delegate.StoreClientTestHelper;
import org.astrogrid.store.delegate.myspace.MySpaceIt04Delegate;

/**
 * NOTE this is an environment/integration test - it needs the two
 * servers set up as given below
 *
...
public class MySpaceIt04DelegateTest extends StoreClientTestHelper {
   ...
   private static final String STREAMFILE = "streamtest.txt";

   public void setUp() {
      path = "avodemo@test.astrogrid.org/serv1/scratch/";
   }
   ...
   public void testStream() throws IOException {
      StoreClient client = new MySpaceIt04Delegate(User.ANONYMOUS, TESTSTORE);
      
      try {
         client.delete(path+STREAMFILE);
      }
      ...
      DataInputStream in = new DataInputStream(client.getStream(path+STREAMFILE));
      for (int i=0;i<900;i++) {
         String s = in.readLine();
         LogFactory.getLog(MySpaceIt04DelegateTest.class).debug(s);
         assertTrue("Failed on line "+i, s.startsWith("This is an automatic streaming test "+i));
      }
      in.close();
