<?xml version="1.0"?>
<!--+
    | <cvs:source>$Source: /Users/pharriso/Work/ag/repo/git/astrogrid-mirror/astrogrid/old-portal/src/site/xsl/myspace/Attic/lookup-request.xsl,v $</cvs:source>
    | <cvs:date>$Author: dave $</cvs:date>
    | <cvs:author>$Date: 2003/06/03 13:16:47 $</cvs:author>
    | <cvs:version>$Revision: 1.1 $</cvs:version>
    | <cvs:log>
    | $Log: lookup-request.xsl,v $
    ...
    | </cvs:log>
    | 
    | XSL template to convert SOAP request into XInclude element.
    | Looks for the request/query/path element.
    | 
    +-->
<xsl:stylesheet
	version="1.0" 
	...
	<xsl:template match="request/content//query">
		...
		<xi:include>
			<xsl:attribute name="href">
				<xsl:text>xml/myspace/lookup-data.xml#xpointer(</xsl:text><xsl:value-of select="path"/><xsl:text>)</xsl:text>
			</xsl:attribute>
		</xi:include>
	</xsl:template>
