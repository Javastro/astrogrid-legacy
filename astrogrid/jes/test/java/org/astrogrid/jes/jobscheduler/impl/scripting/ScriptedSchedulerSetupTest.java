/*$Id: ScriptedSchedulerSetupTest.java,v 1.1 2004/07/09 09:32:12 nw Exp $
 ...
 *
 */
public class ScriptedSchedulerSetupTest extends AbstractTestForSchedulerImpl {
    /** Construct a new ScriptedSchedulerSetupTest
     * @param arg0
     */
    public ScriptedSchedulerSetupTest(String arg0) {
        super(arg0);
    }
    /**
    ...
    protected void performTest(JobURN urn) throws Exception {
        ...
        ScriptedSchedulerImpl sched = (ScriptedSchedulerImpl)scheduler;
        ...
        for (int i = 0; i < arr.length; i++) {
            Extension e = arr[i];
            if (e.getKey().equals(WorkflowInterpreter.EXTENSION_KEY)) {
                foundPickle = true;
        ...
        InterpreterEnvironment env = new InterpreterEnvironment(wf1,dispatcher); 
        

        WorkflowInterpreter interp = (new WorkflowInterpreterFactory(new DevelopmentJarPaths())).unpickleFrom(env);
        assertNotNull(interp);
        interp.pyInterpreter.exec("interp.findNext()"); // will throw if interp is not recognized
...
/* 
$Log: ScriptedSchedulerSetupTest.java,v $
