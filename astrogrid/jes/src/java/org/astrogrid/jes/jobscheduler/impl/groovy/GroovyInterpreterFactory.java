/*$Id: GroovyInterpreterFactory.java,v 1.7 2005/04/25 12:13:54 clq2 Exp $
...
/**
 * Factory object that takes care of creating fresh and unpickling existing
 * groovy interpreters.
 * 
...
public class GroovyInterpreterFactory {
    /**
     * interface to a component that actually performs the serialization /
     * deserialization in some way.
     * 
    ...
    public interface Pickler {
        ...
        /**
         * serialize / pickle an intepreter (i.e. all hte state of an executing
         * workflow) to a writer
         * 
         * @param out
         * @param interp
         * @throws Exception
        ...
        /**
         * unmarshal a previously pickled rulestore (subcomponent of an
         * intepreter).
         * 
         * @param reader
         * @return
    ...
    public GroovyInterpreterFactory(Pickler p, TemporaryBuffer buff) {
        this.pickler = p;
        this.buff = new SoftReference(buff);
        this.interpreterCache = new Cache(CACHE_SIZE);
    }

    protected final Pickler pickler;

    protected Reference buff;

    protected final Cache interpreterCache;

    public static final int CACHE_SIZE = 5;

    /** so our buffer can be shared amongst friends.. */
    public synchronized TemporaryBuffer getBuffer() {
        TemporaryBuffer b = (TemporaryBuffer) buff.get();
        if (b == null) {
            b = new TemporaryBuffer();
            buff = new SoftReference(b);
        }
        return b;
    }
    ...
    public void pickleTo(GroovyInterpreter interp, Workflow wf) throws PickleException {
        ...
        Extension pickleJar = (Extension) wf
                .findXPathValue(GroovyInterpreterFactory.EXTENSION_XPATH);
        if (pickleJar == null) {
        ...
        TemporaryBuffer buff = getBuffer();
        buff.writeMode();
        ...
        // finally cache it too.
        interpreterCache.stuff(wf.getJobExecutionRecord().getJobId(), interp);
    ...
    public GroovyInterpreter unpickleFrom(JesInterface env) throws PickleException {
        ...
        Workflow workflow = env.getWorkflow();
        JobURN urn = workflow.getJobExecutionRecord().getJobId();
        GroovyInterpreter gi = (GroovyInterpreter) interpreterCache.check(urn);
        if (gi != null) { // found a match in the cache - that's enough
            return gi;
        }
        Extension pickleJar = (Extension) workflow
                .findXPathValue(GroovyInterpreterFactory.EXTENSION_XPATH);
        StringReader sr = new StringReader(pickleJar.getContent());
        try {
            gi = pickler.unmarshallInterpreter(sr);
    ...
    public GroovyInterpreter newInterpreter(String rulesDoc, JesInterface env)
 ...
 * jes-nww-776-again
 *
 * Revision 1.6.56.1  2005/04/12 17:08:15  nw
 * caching to improve performance
 * Revision 1.6 2004/11/05 16:52:42 jdt
 * Merges from branch nww-itn07-scratchspace
 * 
 * Revision 1.5.18.1 2004/11/05 16:13:08 nw replaced stringWriter with
 * TemporaryBuffer updated to work with new ruleStore
 * 
 * Revision 1.5 2004/09/16 21:47:56 nw made sure all streams are closed
 * 
 * Revision 1.4 2004/09/06 16:30:25 nw javadoc
 * 
 * Revision 1.3 2004/08/09 17:32:18 nw updated due to removing RuleStore
 * 
 * Revision 1.2 2004/07/30 15:42:34 nw merged in branch nww-itn06-bz#441 (groovy
 * scripting)
 * 
 * Revision 1.1.2.2 2004/07/28 16:24:23 nw finished groovy beans. moved useful
 * tests from old python package. removed python implemntation
 * 
 * Revision 1.1.2.1 2004/07/27 23:37:59 nw refactoed framework. experimented
 * with betwixt - can't get it to work. got XStream working in 5 mins. about to
 * remove betwixt code.
 * 
 * Revision 1.1.2.1 2004/07/26 15:51:19 nw first stab at a groovy scheduler.
 * transcribed all the classes in the python prototype, and took copies of the
 * classes in the 'scripting' package.
 * 
 * Revision 1.1 2004/07/09 09:30:28 nw merged in scripting workflow interpreter
 * from branch nww-x-workflow-extensions
 * 
 * Revision 1.1.2.1 2004/05/21 11:25:19 nw first checkin of prototype scrpting
