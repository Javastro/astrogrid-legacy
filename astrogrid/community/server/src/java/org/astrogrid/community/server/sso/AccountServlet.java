...
public class AccountServlet extends HttpServlet {
    ...
    try {
      ...
      if (path.startsWith("/")) {
        if (path.endsWith("/proxy")) {
          ...
          changePassword(request, userName, response);
        }
      }
      else {
  ...
  /**
   * Signs a user into the community, supplying credentials.
   */
  private void signOn(HttpServletRequest  request,
                      String              userName,
  ...
  protected void changePassword(HttpServletRequest  request,
    ...
    if (newPassword == null) {
      ...
      cs.authenticate(userName, oldPassword);
