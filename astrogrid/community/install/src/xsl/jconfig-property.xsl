<?xml version="1.0"?>
<!--+
    | <cvs:source>$Source: /Users/pharriso/Work/ag/repo/git/astrogrid-mirror/astrogrid/community/install/src/xsl/Attic/jconfig-property.xsl,v $</cvs:source>
    | <cvs:author>$Author: dave $</cvs:author>
    | <cvs:date>$Date: 2004/02/20 21:11:05 $</cvs:date>
    | <cvs:version>$Revision: 1.2 $</cvs:version>
    | <cvs:log>
    |   $Log: jconfig-property.xsl,v $
...
<xsl:stylesheet
    ...
    >
    <!--+
        | Config params from the Ant build.
        +-->
    <xsl:param name="property.category"/>
    <xsl:param name="property.name"/>
    <xsl:param name="property.value"/>

    <!--+
        | Process a matching property.
        +-->
    <xsl:template match="/properties/category[@name=$property.category]/property[@name=$property.name]">
        <xsl:copy>
