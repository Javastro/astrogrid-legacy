/*
 $Id: JVot.java,v 1.1 2002/11/27 16:58:47 mch Exp $
...
public class JVot extends JTable implements ActionListener
{
   private VOTableUtil.Table vot = null;
   ...
   public void setVot(VOTableUtil.Table givenVot)
   {
      vot = givenVot;

      DefaultTableModel model = (DefaultTableModel) getModel();
      for(int i=0; i<vot.getFieldCount(); i++)
      {
         VOTableUtil.Field field = (VOTableUtil.Field)vot.getFieldAt(i);

         model.addColumn(field.getName()+" ["+field.getUcd()+"]");
      }

      VOTableUtil.Tabledata data = vot.getData().getTabledata();
