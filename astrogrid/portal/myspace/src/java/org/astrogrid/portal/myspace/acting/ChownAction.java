...
public class ChownAction extends AbstractAction {
  ...
  public Map act(Redirector redirector, SourceResolver resolver, Map objectModel, String source, Parameters params) {
    ...
    try {
      ...
      String newOwner = utils.getAnyParameter("myspace-chown", params, request, session); 
      logger.debug("[act] newOwner: " + newOwner);

      renameResult = delegate.changeOwner(userId, communityId, credential, oldMySpaceName, newOwner);
      
      request.setAttribute("myspace-chown", "true");
      request.setAttribute("myspace-chown-result", renameResult);
    }
    catch(Throwable t) {
      request.setAttribute("myspace-chown", "false");
      request.setAttribute("myspace-chown-error-message", t.getLocalizedMessage());
      sitemapParams = null;
