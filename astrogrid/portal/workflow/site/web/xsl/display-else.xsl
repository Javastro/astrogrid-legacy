...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!--+
       | Display else details
       |
       | Display empty table...
       |
       +-->
    <xsl:template match="elseObj" mode="activity-details">
      <div style="display: none"><xsl:attribute name="id"><xsl:value-of select="@key"/></xsl:attribute> 
        <form name="else_form" id="else_form">
          <table width="10%"  border="2" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td>
                <div style="color: blue; background-color: lightblue; text-align: center;"> Else: </div>
              </td>
            </tr>
            <tr>
              <td colspan="2">
                ...
                <xsl:attribute name="id">helpDiv<xsl:value-of select="@key"/></xsl:attribute>                    
                  The if element may have either or both a then and <b>else</b> child elements.<br /> 
  ...
  <!--+
       | Display else details for job status page
       |
       | Display empty table...
       |
       +-->
    <xsl:template match="elseObj" mode="job-status">
      <div style="display: none"><xsl:attribute name="id"><xsl:value-of select="@key"/></xsl:attribute> 
        <form name="else_form" id="else_form">
          <table width="10%"  border="2" cellspacing="0" cellpadding="0">
            <tr>                
