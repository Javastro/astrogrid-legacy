...
public class MySpaceStressTest extends TestCase {
	...
	private static final String TEST_USER = "jdturl";
	...
	public void setUp() throws IOException {
		MYSPACE = new Agsl(
				"astrogrid:store:myspace:http://twmbarlwm.star.le.ac.uk:8888/astrogrid-mySpace-SNAPSHOT/services/Manager");
		//MYSPACE = ConfManager.getInstance().getMySpaceEndPoint();

		//MYSPACE = new
		// Agsl("myspace:http://vm05.astrogrid.org:8080/astrogrid-mySpace/services/MySpaceManager");
		//MYSPACE = new
		// Agsl("myspace:http://grendel12.roe.ac.uk:8080/MySpaceManager_v041/services/MySpaceManager");
		...
		try {
			StoreDelegateFactory.createAdminDelegate(testUser, MYSPACE)
					.createUser(testUser);
	...
	public void testStickFileInByURL() throws IOException {

		StoreClient aStore = StoreDelegateFactory.createDelegate(testUser,
				MYSPACE);
		...
		for (int i = 0; i < MAX; ++i) {
			...
			URL url = new URL("http://software.astrogrid.org/");
			aStore.putUrl(url, path + "fileFromURL"
		...
		//    assertEquals(aStore.getFile(path+SOURCE_TEST).getPath(),
		// sameStore.getFile(path+SOURCE_TEST).getPath());
	...
	public void testStickFileIn() throws IOException {
		...
		for (int i = 0; i < MAX; ++i) {
			...
			System.out.print("Putting file " + i + "of length" + bytes.length
					+ "...");
			aStore.putBytes(bytes, 0, bytes.length, path + "aBigFile"
					+ Integer.toString(i), false);
			System.out.println("...done");
		}
