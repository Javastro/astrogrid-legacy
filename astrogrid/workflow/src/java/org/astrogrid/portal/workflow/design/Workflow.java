...
public class Workflow extends Activity {
  ...
  public static Workflow createWorkflow(
    ...
    String description) {
    if (TRACE_ENABLED)
      trace("Workflow.createWorkflow() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.createWorkflow() exit");
    }
  ...
  public static Workflow createWorkflowFromTemplate(
    String communitySnippet,
    String name,
    String description,
    String templateName) {
    if (TRACE_ENABLED)
      trace("Workflow.createWorkflowFromTemplate() entry");
    ...
    try {
      ...
      InputSource source =
        new InputSource(new StringReader(retrieveTemplate(templateName)));
      ...
      if (TRACE_ENABLED)
        trace("Workflow.createWorkflowFromTemplate() exit");
    }
  ...
  public static Workflow readWorkflow(String communitySnippet, String name) {
    if (TRACE_ENABLED)
      trace("Workflow.readWorkflow() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readWorkflow() exit");
    }
  ...
  public static boolean deleteWorkflow(String communitySnippet, String name) {
    if (TRACE_ENABLED)
      trace("Workflow.deleteWorkflow() entry");

    boolean retValue = true; // Assumption is it works!
    StringBuffer pathBuffer = new StringBuffer(64);
    String mySpaceLocation = null, account = null;
    ...
    try {
      ...
      if (mySpaceLocation == null || mySpaceLocation.trim().equals("")) {
        ...
        MySpaceClient mySpace =
          MySpaceDelegateFactory.createDelegate(mySpaceLocation);
        ...
        mySpace.deleteDataHolding(
          Workflow.extractUserid(account),
      ...
      if (TRACE_ENABLED)
        trace("Workflow.deleteWorkflow() exit");
    }
  ...
  public static boolean saveWorkflow(
    String communitySnippet,
    Workflow workflow) {
    if (TRACE_ENABLED)
      trace("Workflow.saveWorkflow() entry");

    boolean retValue = false;
    String xmlWorkflow = null,
      filePath = null,
...
"workflow" //@TODO JDT according to the myspace api this ought to be WF            // it's a workflow
  , "Overwrite"); // overwrite it if it already exists
      ...
      if (TRACE_ENABLED)
        trace("Workflow.saveWorkflow() exit");
    }
  ...
  public static boolean submitWorkflow(
    String communitySnippet,
    Workflow workflow) {
    if (TRACE_ENABLED)
      trace("Workflow.submitWorkflow() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.submitWorkflow() exit");
    }
   ...
   * of the files.
   */
  public static Iterator readWorkflowList(
    String communitySnippet,
    String filter) {
    if (TRACE_ENABLED)
      trace("Workflow.readWorkflowList() entry");
    ...
    try {
      ...
      if (mySpaceLocation == null || mySpaceLocation.trim().equals("")) {
        ...
        // This is here purely for test situations...
        iterator =
          WorkflowHelper.readWorkflowList(communitySnippet, filter).iterator();
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readWorkflowList() exit");
    }
  ...
  public static Iterator readToolList(String communitySnippet) {
    if (TRACE_ENABLED)
      trace("Workflow.readToolList() entry");
    ...
    try {
      ...
      numberTools =
        new Integer(
          WKF.getProperty(WKF.TOOLS_LIST_TOTAL, WKF.TOOLS_LIST_CATEGORY))
          .intValue();
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readToolList() exit");
    }
  ...
  public static Tool createTool(String communitySnippet, String name) {
    if (TRACE_ENABLED)
      trace("Workflow.createTool() entry");
    ...
    try {
      ...
      numberParams =
        new Integer(WKF.getProperty(WKF.TOOL_INPUT_PARAMS_TOTAL, name))
          .intValue();
      debug("Number of input params: " + numberParams);
      for (int i = 0; i < numberParams; i++) {
        param =
          tool.newInputParameter(
            WKF.getProperty(WKF.TOOL_INPUT_PARAM_NAME + i, name));
        param.setDocumentation(
          WKF.getProperty(
            WKF.TOOL_INPUT_PARAM_DOCUMENTATION + param.getName(),
            name));
        param.setType(
          WKF.getProperty(WKF.TOOL_INPUT_PARAM_TYPE + param.getName(), name));
        cardinality =
          new Cardinality(
            WKF.getProperty(
              WKF.TOOL_INPUT_PARAM_CARDINALITY_MIN + param.getName(),
              name),
            WKF.getProperty(
              WKF.TOOL_INPUT_PARAM_CARDINALITY_MAX + param.getName(),
              name));
      ...
      numberParams =
        new Integer(WKF.getProperty(WKF.TOOL_OUTPUT_PARAMS_TOTAL, name))
          .intValue();
      debug("Number of output params: " + numberParams);
      for (int i = 0; i < numberParams; i++) {
        param =
          tool.newOutputParameter(
            WKF.getProperty(WKF.TOOL_OUTPUT_PARAM_NAME + i, name));
        param.setDocumentation(
          WKF.getProperty(
            WKF.TOOL_OUTPUT_PARAM_DOCUMENTATION + param.getName(),
            name));
        param.setType(
          WKF.getProperty(WKF.TOOL_OUTPUT_PARAM_TYPE + param.getName(), name));
        cardinality =
          new Cardinality(
            WKF.getProperty(
              WKF.TOOL_OUTPUT_PARAM_CARDINALITY_MIN + param.getName(),
              name),
            WKF.getProperty(
              WKF.TOOL_OUTPUT_PARAM_CARDINALITY_MAX + param.getName(),
              name));
        param.setCardinality(cardinality);
        if (cardinality.getMinimum() > 1) {
          for (int j = 0; j < cardinality.getMinimum() - 1; j++) {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.createTool() exit");
    }
  ...
  public static Iterator readQueryList(
    String communitySnippet,
    String filter) {
    if (TRACE_ENABLED)
      trace("Workflow.readQueryList() entry");
    ...
    try {
      ...
      if (mySpaceLocation == null || mySpaceLocation.trim().equals("")) {
        ...
        // This is here purely for test situations...
        iterator =
          WorkflowHelper.readQueryList(communitySnippet, filter).iterator();
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readQueryList() exit");
    }
  ...
  public static String readQuery(String communitySnippet, String name) {
    if (TRACE_ENABLED)
      trace("Workflow.readQuery() entry");
  ...
  private String name, description, templateName;
  ...
  private ActivityContainer child;
  ...
  public Workflow(String communitySnippet, Document document) {
    ...
    try {
      ...
      this.templateName =
        element.getAttribute(WorkflowDD.WORKFLOW_TEMPLATENAME_ATTR);
      ...
      for (int i = 0; i < nodeList.getLength(); i++) {
        ...
        if (nodeList.item(i).getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (element.getTagName().equals(WorkflowDD.DESCRIPTION_ELEMENT)) {
            ...
            element.getTagName().equals(WorkflowDD.SEQUENCE_ELEMENT)) {
  ...
  public Activity getActivity(String key) {
    if (TRACE_ENABLED)
      trace("Workflow.getActivity() entry");
    try {
      if (TRACE_ENABLED)
        debug("key: [" + key + "]");
      if (TRACE_ENABLED)
        debug("activities: " + activities.toString());
      return (Activity) activities.get(new ActivityKey(key));

    } finally {
      if (TRACE_ENABLED)
        trace("Workflow.getActivity() exit");
    }
  }

  public boolean putActivity(Activity activity) {
    if (TRACE_ENABLED)
      trace("Workflow.putActivity() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.putActivity() exit");
    }
  ...
  protected String constructWorkflowXML(String communitySnippet) {
    if (TRACE_ENABLED)
      trace("Workflow.constructWorkflowXML() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.constructWorkflowXML() exit");
    }
  ...
  protected String constructJESXML(String communitySnippet) {
    if (TRACE_ENABLED)
      trace("Workflow.constructJESXML() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.constructJESXML() exit");
    }
  ...
  protected static String retrieveTemplate(String templateName) {
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.retrieveTemplate() exit");
    }
  ...
  protected static String extractCommunity(String account) {
    String retVal = account.substring(account.indexOf("@") + 1);
   ...
   * of the files.
   */
  protected static Iterator readMySpaceList(
    String communitySnippet,
    String myspaceLocation,
    String myspaceArguments) {

    if (TRACE_ENABLED)
      trace("Workflow.readMySpaceList() entry");
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readMySpaceList() exit");
    }
  ...
  protected static String readMySpaceFile(
    String communitySnippet,
    String myspaceLocation,
    String arguments) {
    if (TRACE_ENABLED)
      trace("Workflow.readMySpaceFile() entry");

    String account = null, fileString = null;
    ...
    try {
      ...
      if (TRACE_ENABLED)
        trace("Workflow.readMySpaceFile() exit");
    }
  ...
  /**
    ...
    * @see 
    **/
  public static String formatMySpaceURL(
    String communitySnippet,
    ...
    buffer.append("myspace://").append(logicalDirectoryPath).append(
      "/").append(
      fileName);
