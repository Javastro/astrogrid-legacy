/*$Id: HttpApplicationDescription.java,v 1.8 2008/09/03 14:19:02 pah Exp $
...
public class HttpApplicationDescription extends AbstractApplicationDescription {
     ...
     * Constructor
     */
    public HttpApplicationDescription(CeaHttpApplicationDefinition application, Resource res,
            final ApplicationDescriptionEnvironment env, final Configuration config) {
        super(application, res, env);
        this.config = config;
        if (log.isTraceEnabled()) {
            log.trace("HttpApplicationDescription(CeaHttpApplication application = " + application
    ...
    public Application initializeApplication(String jobStepID, User user, Tool tool) throws Exception {
        ...
        ApplicationEnvironment appenv = new ApplicationEnvironment(jobStepID, user, getInternalComponentFactory().getIdGenerator(), config );
	Application returnApplication = new HttpApplication(tool, appInterface, appenv , getInternalComponentFactory().getProtocolLibrary());
