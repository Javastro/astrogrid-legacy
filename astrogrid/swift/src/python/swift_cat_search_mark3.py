...
class ConeSearch:
	...
	def __init__( self, ivorn ):
		...
		ConeSearch.ar = xmlrpclib.Server( prefix + "xmlrpc" )
		self.ivorn = ivorn
		#
		#import service we're going to use from ar.
		ConeSearch.tables = ConeSearch.ar.util.tables
	...
	def execute( self, ra, dec, radius, dsatab=None ):	
		# use AR to build the query URL       
		query = ConeSearch.ar.ivoa.cone.constructQuery( self.ivorn, ra, dec, radius )
		fullURL = ConeSearch.ar.ivoa.cone.addOption( query, "VERB", "3" )
		doc = ConeSearch.tables.convertFromFile( fullURL, "votable", "votable" )
		return doc
...
#	if log: print 'formNullValuesMap() enter'
	nullVals = {}
	...
	#print '=====nullVals====='
#	print nullVals
#	if log: print 'formNullValuesMap() exit'
	return nullVals

def testAndSetNull( x, index, nullsMap ):
#	if log: print 'testAndSetNull() enter'
	...
	if x != None:
		if str(x) != 'NaN':
			if nullsMap.has_key( index ) == True:
				...
				retVal = x
#	print retVal
#	if log: print 'testAndSetNull() exit'
	return retVal
