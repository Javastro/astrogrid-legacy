 ...
 */
public abstract class AbstractResourceActivity extends AbstractSingleActivity implements Predicate {
	/**
	...
	public void selected(Transferable r) {
		...
		if (r.isDataFlavorSupported(VoDataFlavour.LOCAL_RESOURCE)) {
			...
			someSelected((Resource[])r.getTransferData(VoDataFlavour.LOCAL_RESOURCE_LIST));
		} else if (r.isDataFlavorSupported(VoDataFlavour.RESOURCE_LIST)) {
			someSelected((Resource[])r.getTransferData(VoDataFlavour.RESOURCE_LIST));
		} else {
			noneSelected();
		}
		} catch (IOException x) {
