...
public class SessionManagerIntegrationTest extends InARTestCase {
	...
	public void testDefaultSession() throws Exception {
		final String id = sess.getDefaultSessionId();
		assertNotNull(id);
		assertTrue(sess.exists(id));
		final Principal p = sess.findSessionForKey(id);
		assertNotNull(p);
	}
	...
	public void testCreateNewSession() throws Exception {
		final String id = sess.createNewSession(20); //20 mins
		assertNotNull(id);
		assertTrue(sess.exists(id));
	...
	public void testNewSessionHttp() throws Exception {
		...
		assertNotNull(id);
		assertTrue(sess.exists(id));
		final URL u = sess.findHttpSession(id);
		assertNotNull(u);
		// now need to test this.
		final WebTester web = new WebTester();
	...
	public void testNewSessionXmlRpc() throws Exception {
		...
		assertNotNull(id);
		assertTrue(sess.exists(id));
		final URL u = sess.findXmlRpcSession(id);
		assertNotNull(u);
		final URL defaultU = sess.findXmlRpcSession(sess.getDefaultSessionId());
		
		// check some things are shared.
		final Configuration conf = new ConfigurationXmlRpcClient(defaultU);
		final Configuration sConf = new ConfigurationXmlRpcClient(u);
		checkSharedService(id, conf, sConf);

		// and test some things are sessioned.
		final SessionTest st = new SessionTestXmlRpcClient(defaultU);
		final SessionTest sst = new SessionTestXmlRpcClient(u);	
	...
	public void testNewSessionRmi() throws Exception {
		...
		final ACR acr = f.find();
		assertTrue(acr instanceof RmiAcr);
		final ACR sessioned = f.findSession(id);
		...
		// check some things are shared.
		final Configuration conf = (Configuration)acr.getService(Configuration.class);
		final Configuration sConf =(Configuration)sessioned.getService(Configuration.class);
		assertNotSame(conf,sConf);
		// we'll use id as a suitable temporary key.
		checkSharedService(id, conf, sConf);
		
		// check other things are sessioned.
		final SessionTest st = (SessionTest)acr.getService(SessionTest.class);
		final SessionTest sst = (SessionTest)sessioned.getService(SessionTest.class);
		checkSessionedService(st, sst);
	...
	public static class SessionTestXmlRpcClient implements SessionTest {
		...
		public SessionTestXmlRpcClient(final URL rpcUrl) {
			client = Fixture.createXmlRpcClient(rpcUrl);		
		}	
		public final org.apache.xmlrpc.client.XmlRpcClient client;
		public String get() {
			try {
			final String s = (String)client.execute("test.sessiontest.get",new Object[]{});
	...
	private void checkSharedService(final String id, final Configuration conf, final Configuration sConf) {
	...
	private void checkSessionedService(final SessionTest st, final SessionTest sst) {
		assertNotNull(st);
		assertNotNull(sst);
