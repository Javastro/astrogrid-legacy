...
// Paul Harrison - note that this class contains extensions for ACR type interoperability - C++ just made it too difficult to do these in a subclass and retain simple syntax for use
// specifically ran across "invalid initialization of a non-const blah from a temporary of type blah " problems - would be nice to actually get the ACR specific functionality into a subclass in future, but it does really seem like harder work...
...
namespace XmlRpc {
  ...
  public:
   ...
   /* virtual */ ~XmlRpcValue() { invalidate(); }
    ...
    operator long&()          { assertTypeOrInvalid(TypeLong); return _value.asLong;}
  ...
  private:
	  static std::string intToXmlHelper( int const val);
  };
} // namespace XmlRpc
