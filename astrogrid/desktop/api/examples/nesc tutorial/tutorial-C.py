...
targetService = acr.dialogs.registryChooser.chooseResourceWithFilter('Keyword search for SIAP services', "@xsi:type like '%SimpleImageAccess' and @status = 'active'")
if targetService == 'null':
	raise Exception, "no registry entry selected"

#registry key of the target service.
targetServiceId = targetService['id']
print "Enter object name"
objName = sys.stdin.readline()
sesameXML = acr.cds.sesame.sesame(objName,'x')
sesameDoc = Sax2.Reader().fromString(sesameXML)
ra = xpath.Evaluate('number(/Sesame/Resolver/jradeg)',sesameDoc.documentElement)
dec= xpath.Evaluate('number(/Sesame/Resolver/jdedeg)',sesameDoc.documentElement)
...
# calculate root storage location.
rootIvorn = acr.astrogrid.myspace.getHome()
resultDir = rootIvorn + "votable"
...
for r in xpath.Evaluate('/VOTABLE/RESOURCE/TABLE/DATA/TABLEDATA/TR',el):
	...
	print title
	print format
	# create a filename, based on what we know of the image - need to remove non alnum characters.
	ivorn= resultDir + "/" +  filter(lambda x: x.isalnum(),title) + "." + format.split("/")[1]
	# build up html table row.
	ivornParam = urllib.urlencode([('ivorn',ivorn)])
	urlParam = urllib.urlencode([('source',imgURL)])
	html = html + """
<tr><td>%(title)s</td>
    <td>%(format)s</td>
    <td><a href="%(url)s">view</a></td>
<td><a href="%(prefix)sastrogrid/myspace/copyURLToContent/plain?%(urlParam)s&%(ivornParam)s">save</a></td>    
</tr>
""" % {'title':title, 'format':format, 'urlParam':urlParam, 'url':imgURL, 'prefix':prefix,'ivornParam':ivornParam}
...
acr.system.browser.openURL(resultUrl)
