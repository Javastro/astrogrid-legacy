...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<!-- 
		...
		Current stylesheet was derived from POSTGRES-7.2.3.xsl
	 -->
   ...
   -->
	<xsl:template match="*[ contains(@xsi:type, 'binaryExprType') ]">
		...
		<xsl:text> </xsl:text>
		<xsl:choose>
         	<!-- Deal with bitwise XOR as special case -->
         	<xsl:when test="@Oper = '^'">
            	<xsl:text>#</xsl:text>
         	</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="*[ contains(@xsi:type, 'integerType') ]">
		<xsl:value-of select="@Value"/>
	</xsl:template>
	
	<xsl:template match="*[ contains(@xsi:type, 'realType') ]">
	...
	<xsl:template match="*[ contains(@xsi:type, 'mathFunctionType') ]" >
      <xsl:choose>
	...
	<!--+
	    | These require further study if we must CAST some of the arguments.
	    | Which require casting? 
	    | For userDefinedFunctionType we simply don't know without some indication
	    | of what is or is not numeric!
	    +-->
