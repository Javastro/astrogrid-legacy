...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	version="1.0" xmlns="http://www.ivoa.net/xml/v2.0/adql" xmlns:ad="http://www.ivoa.net/xml/v2.0/adql"
	xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">

	<!--
		Stylesheet to convert ADQL/x v2.0 to an SQL string derived from an
		original by Aurelien STEBE - ESAC - ESA Aurelien.Stebe@sciops.esa.int
	-->
	...
	<xsl:template match="ad:Select">
		...
		<!-- if where not set, handle restrict -->
		<xsl:choose>
			<xsl:when test="ad:Where">
				<xsl:apply-templates select="ad:Restrict">
					<xsl:with-param name='restrictWord' select="'And'" />
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="ad:Restrict">
					<xsl:with-param name='restrictWord' select="'Where'" />
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	...
	<!--
		+ | NB: This is about as close as one dare use the contains function
		rather than | the substring-after function and be able to rely upon
		case sensitivity | to do the correct thing. +
	-->
	...
	<xsl:template
		match="*[ contains(@xsi:type, 'joinTableType') ] | ad:JoinedTable">
		...
		<xsl:choose>
			<xsl:when test="$joinType='LEFT_OUTER'">
				<xsl:text> LEFT OUTER JOIN </xsl:text>
			</xsl:when>
			<xsl:when test="$joinType='RIGHT_OUTER'">
				<xsl:text> RIGHT OUTER JOIN </xsl:text>
			</xsl:when>
			<xsl:when test="$joinType='FULL_OUTER'">
				<xsl:text> FULL OUTER JOIN </xsl:text>
			</xsl:when>
			<xsl:when test="$joinType='INNER'">
	...
	<xsl:template
		match="*[ contains(@xsi:type, 'comparisonPredType') ] | *[@Comparison]">
	...
	<!--
		KEA: - Added brackets around binary expressions to ensure correct
		precendence - Put spaces around operator to avoid issues with e.g.
		subtracting a negated arg. (SQLServer doesn't like doubled - with no
		spaces)
	-->
	...
	<xsl:template
		match="*[ contains(@xsi:type, 'columnReferenceType') ] | ad:Column">
		<xsl:call-template name="qualifiedName" />
	</xsl:template>
	...
	<xsl:template
		match="*[ contains(@xsi:type, 'realType') ] | *[ contains(@xsi:type, 'integerType') ]">
	...
	<xsl:template match="ad:Restrict">
		<xsl:param name='restrictWord' />
		<xsl:value-of select="$restrictWord" />
		<xsl:value-of select="$spaceCharacter" />
		<xsl:text>rownum&lt;=</xsl:text>
	...
	<xsl:template match="*[ contains(@xsi:type, 'trigonometricFunctionType') ]
     ...
     <!-- Deal with ATAN2 as special case -->
         ...
         <!-- This is a custom action for TRUNCATE function (takes 2 args) -->
	 <!--
         <xsl:when test="@Name = 'TRUNCATE'">
           <xsl:text>TRUNCATE(</xsl:text>
           <xsl:apply-templates select="ad:Allow"/>
           <xsl:apply-templates select="ad:Arg"/>
           <xsl:text>,0)</xsl:text>
           <xsl:value-of select="$spaceCharacter"/>
	 ...
	 -->
         <!-- This is a custom action for LOG function (LOG(base, arg)) -->
         <xsl:when test="@Name = 'LOG10'">
           <xsl:text>LOG(10,</xsl:text>
           <xsl:apply-templates select="ad:Arg"/>
           <xsl:text>)</xsl:text>
           <xsl:value-of select="$spaceCharacter"/>
         </xsl:when>
         <xsl:when test="@Name = 'RADIANS'">
           <xsl:text>(0.0174532925 * </xsl:text>
           <xsl:apply-templates select="ad:Arg"/>
           <xsl:text>)</xsl:text>
           <xsl:value-of select="$spaceCharacter"/>
         </xsl:when>
         <xsl:when test="@Name = 'DEGREES'">
           <xsl:text>(57.2957795 * </xsl:text>
           <xsl:apply-templates select="ad:Arg"/>
           <xsl:text>)</xsl:text>
           <xsl:value-of select="$spaceCharacter"/>
         </xsl:when>
	...
	<!--+
	    ...
	    |       can be qualified by some form of ownership.
