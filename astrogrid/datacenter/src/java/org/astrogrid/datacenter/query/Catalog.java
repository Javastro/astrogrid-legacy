...
public class Catalog {
   ...
   public Catalog( Element catalogElement ) throws QueryException
   {
      ...
      try {
         ...
         setName(catalogElement.getAttribute( AdqlTags.CATALOG_NAME_ATTR )) ;

         NodeList
            nodeList = catalogElement.getElementsByTagName( AdqlTags.TABLE_ELEMENT ) ;
         ...
         for( int i=0 ; i < nodeList.getLength() ; i++ ) {
            ...
            if( tableElement.getTagName().equals( AdqlTags.TABLE_ELEMENT ) ) {
               this.addTable( new Table( tableElement ) ) ;
            }

         } // end for

         nodeList = catalogElement.getElementsByTagName( AdqlTags.SERVICE_ELEMENT ) ;

         for( int i=0 ; i < nodeList.getLength() ; i++ ) {
            serviceElement = (Element) nodeList.item(i) ;
            if( serviceElement.getTagName().equals( AdqlTags.SERVICE_ELEMENT ) ) {
