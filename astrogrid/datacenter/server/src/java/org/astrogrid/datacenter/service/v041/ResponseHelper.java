...
public class ResponseHelper
{
   ...
   public static Document makeQueryStartedResponse(Querier querier) throws Throwable
   {
      ...
      if (querier.getStatus().getState() == QueryState.ERROR)
      {
         log.info("querier in error", ((QuerierError) querier.getStatus()).getCause());
         throw ((QuerierError) querier.getStatus()).getCause();
      }

      String doc =
          "<"+QUERY_STARTED_RESP_TAG+">\n"
         +"   "+QueryIdHelper.makeQueryIdTag(querier.getId())
         +"   "+StatusHelper.makeStatusTag(querier.getStatus().getState())
         +"</"+QUERY_STARTED_RESP_TAG+">\n";
   ...
   public static Document makeStatusResponse(Querier querier) throws Throwable
   {
      ...
      String doc =
         StatusHelper.makeStatusTag(querier.getId(), querier.getStatus().getState());
   ...
   public static Document makeResultsResponse(Querier querier, Element results)
   {
      ...
      String doc =
          QueryIdHelper.makeTagWithQueryIdAttr(DATACENTER_RESULTS_TAG, querier.getId())+"\n"
