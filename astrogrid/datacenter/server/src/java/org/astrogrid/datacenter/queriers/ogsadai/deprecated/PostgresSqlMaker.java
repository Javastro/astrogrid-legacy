...
public class PostgresSqlMaker extends StdSqlMaker {
   ...
   public String fromAdql(AdqlQuery query) {
      ...
      try {
        ...
        System.out.println("FLAG PSM 1");
        Translator trans = new AdqlQueryTranslator();
        // do the translation
		System.out.println("FLAG PSM 2");
        Object intermediateRep = null;
		System.out.println("FLAG PSM 3");
		//Select query = Select.unmarshalSelect(reader);
        //Trust it anyway.  ECA 01/04/04 20:54
		try {
		QueryTranslator visitor = new AdqlQueryTranslator();
		System.out.println("FLAG PSM 4");
		intermediateRep = trans.translate(queryBody);
		System.out.println("FLAG PSM 5");
		//String sql = visitor.translate(queryBody);
		//String sql2 = trans.translate(queryBody);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("PSM Caught Exception " + e.getMessage());
		}
		
		/* COMMENTED OUT - ECA 02/04/04 01:45 */
		
        try {
			intermediateRep = trans.translate(queryBody);
        }catch (Throwable t) {
			throw new RuntimeException("In PSM, AdqlQueryTranslator failed:" + t.getMessage());
	    }        
		// return (String) intermediateRep;
		
		/* COMMENTED OUT - ECA 01/04/04 20:55
