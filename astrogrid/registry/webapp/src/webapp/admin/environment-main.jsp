...
<p>
	    <strong>BackGround Info:</strong> For webapps properties are stored in the WEB-INF/web.xml but this xml contains a lot more than just 
...
<p>
Editing properties does not normally require a restart of tomcat, but tomcat 5.5 does not seem to catch the
environment entry changes which requires a restart of the tomcat.  Use the 'Fingerprint' in the menu to check all
the current properties.<br />
</p>
<p>
Many other servlet containers should provide some kind of environment entry editor themselves and is recommended for changing entries, but you may
try the below link to directly edit the Environment Entries in the web.xml if your servlet container does not give you an editor.  It will require a restart of your
servlet container (this will not work for all servlet containers because some containers may move the web.xml to a
different location espcially for containers that do not unpack the war file)<br />
<a href-"viewProperties.jsp">View and edit the web.xml</a>
</p>
