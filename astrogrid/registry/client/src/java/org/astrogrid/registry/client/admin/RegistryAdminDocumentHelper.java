 ...
 * Description: This class is principally used for the generation of a
 * Document object based off of a Mapping object (TreeMap) and vice-versa 
 * going from a  Document Object back to unique strings in a TreeMap.  The 
 * primary use for this is on the Cocoon portal side.  Cocoon can now take a 
 * Document object that it will receive from a server and throw it to this 
 * class to generate a TreeMap object which it can then use to generate Form 
 * Input variables.
 * 
 * @author Kevin Benson
 * 
...
public class RegistryAdminDocumentHelper {
   ...
   private static final Log log = 
                        LogFactory.getLog(RegistryAdminDocumentHelper.class);
  /**
   ...
   */
   ...
   /**
    * Where the creation of the Map begins from a Document object. First it 
    * will create the empty TreeMap object then grab all the ChildNodes from 
    * the Document and then pass it to a recursive method for the generation 
    * of mapping.
    * 
    * @param doc The Document Object to be analysed and parsed into unique 
    *  strings
    * @return A TreeMap which is a mapping of unique strings to their values 
   ...
   /**
    * Recursive method to create the mapping.  Mainly done by iterating 
    * through the NodeList and check if it an Attribute or a Text Node.  If 
    * it is one of those then get all the Parent Node names for generating a 
    * unique string and get the value then input the unique string and value 
    * into a treemap.
    * 
    * @param nl A Nodelist of all the child nodes.
    * @param tm A Treemap Object th contains the mapping of all the unique 
    *  strings with their values.
    */
   private static void createMap(NodeList nl,
                                 Map tm,/*int parentLoopCounter,*/ 
      ...
      for(int i = 0;i < nl.getLength();i++) {
         hadAttributes = false;
         Node nd = nl.item(i);
         //check if the current Node has any Attributes or if it is a Text 
         //Node type
         if((nnm = nd.getAttributes()) != null) {
            hadAttributes = true;
            // Okay Attributes go ahead and get the node/element it lives on 
            // and make the new parent strigns
            parentStrings += nd.getNodeName() + String.valueOf(i) + "/";
            //Okay don't touch the parent strings anymore so make a temp of 
            //the parent strings.
            ...
            for(int j = 0; j < nnm.getLength();j++) {
               ...
               tm.put(temp + "attr/" + attrNode.getNodeName(),
            ...
            if(parentStrings != null && parentStrings.length() > 0) {
               ...
               val = nd.getNodeValue();
               // Sometiems the DOM parse makes empty text nodes for newlines 
               // so make sure we have a real value to the text node.
         ...
         // If the current node has more child nodes then call itself again 
         // with those childnodes.
         if(nd.hasChildNodes()) {
            String newNodeEndingString = "";
            // If we had attributes earlier then the current nodename/element 
            // is already on the parent strings.
            if(!hadAttributes)
               newNodeEndingString = nd.getNodeName() + String.valueOf(i) + 
               "/"; 
            // Add the new parent elements to the parent strings.
            parentStrings += newNodeEndingString;
            
            //Now call itself with the child nodes.
            createMap(nd.getChildNodes(),tm,parentStrings);
            // Okay we came back from a recursive call so get rid of the 
            // previous node name.
            // This makes sure our parent strings go back up the tree.
            parentStrings = parentStrings.substring(0,parentStrings.
                            indexOf(nd.getNodeName() + String.valueOf(i)));
         }//if
      }//for
      log.debug("end createMap");
   }
  ...
  /**
   * This method takes in a mapping and creates a Document object model.  Does
   * this by dealing and creating a custom object called SchemaBuilder which 
   * has child SchemaBuilders and access to attributes.  Then finally a call 
   * to it's generateDocument method in the SchemaBuilder will go through all 
   * it's children and attributes generating a Document object. 
   *
   * THIS METHOD WILL NEED TO BE REWRITTEN.
   *
   * Because I ran into trouble that I really needed to look back at all the 
   * parents above a current node.
   * If it only needed to lookup once it was not to bad, but because of the 
   * Registry it really need to go look up to 2 parents.  Which caused a small
   * hack for the moment.  Need to rework this.
   * 
   ...
   */
   public static Document createDocument(Map tm) {
      ...
      try {
         ...
         String val = null;
         String []elems = null;
         // Okay initialize the first Schema Builder to nothing but nulls. This
         // is just a reference point.         
            ...
            if(elems.length > 0) {
               ...
               for(int i=0;i < elems.length;i++) {
                  ...
                  if("attr".equals(elems[i])) {
                     // So add the next element string which is the attribute 
                     // name to the current schema builder.
                     // Current schemabuilder should have the element that 
                     // goes with this attribute. 
                     i++;
                     currentsb.addAttribute(elems[i],val);
                     // Go ahead and go to the next string.
                     i = elems.length;                    
                  }else {
                     // See if this element is already in the schema builder 
                     // objects.
                     if(sb.hasElement(currentElement,
                                      prevElement,
                                      prevElement2)) {
                        // It is so set the current schema builder to it.
                        hasElement = true;
                        currentsb = sb.findElement(currentElement,
                                                   prevElement,
                                                   prevElement2);
                     }                     
                     if(i != (elems.length - 1)) {
                        // If were not on the last string where the value goes 
                        // then if the element does not exist yet then add it 
                        // as a child schema builder object to the current 
                        // schema builder object.
                        if(!hasElement) {                        
                           currentsb.addElement(elems[i],prevElement);
                           currentsb = sb.findElement(elems[i],
                                                      prevElement,
                                                      prevElement2);
                        }
                     }else {
                     ...
                     // Reset the hasElement to false were going to the next 
                     // string in the list.
                     hasElement = false;
                  }//else
               }//for
      ...
      log.debug("end createDocument");
      return registryDoc;
   }
}

/**
 * A Schema builder class that handles holding it's current element name and 
 * it's attributes along with child objects which are also Schema Builder 
 * objects. 
 * @author Kevin Benson
 * 
 */
class SchemaBuilder {
   private static final Log log = 
                        LogFactory.getLog(SchemaBuilder.class);
   ...
   /**
    * Constructor that sets the current element name and it's parent element 
    * name.
   ...
   /**
    * Main constructor that sets the current element, the value if any and a 
   ...
   /**
    * Checks if the current same element exists already in all the schema 
    * objects. Recursively looking through all the child Schema Builder 
    * Objects.
    *
   ...
   public SchemaBuilder findElement(String element,String prevElement,
      ...
      for(int i = 0;i < childElements.size();i++) {
         ...
         if(temp.equals(current)) {
            ...
            if(current.childElements.size() > 0) {
               ...
               if(!recCheck.getElementName().equals(null)  && 
                  temp.equals(recCheck) )
               {
                  return recCheck;
               }
      ...
      log.debug("end findElement");
      return this;
   }

   /**
    * Equals method that looks only at the elment name.  Probably needs to be 
    * reworked to look at parent name.
   ...
   public Document generateDocument() throws Exception {
      log.debug("start generateDocument");
      ...
      if(childElements.size() > 0) {
         for(int i = 0; i < childElements.size();i++) {
            SchemaBuilder sb = (SchemaBuilder)childElements.get(i);
               sb.generateChildDocument(registryDoc,
                                        registryDoc.getDocumentElement());
         }
      }
      log.debug("end generateDocument");
      return registryDoc;
   }
   
   /**
    * Recursivley calls its child nodes to build up and append the appropriate
    * element names and attributes to add to the DOM object. Recusively builds
    * up the elements from the leaf node back up to the top before then adding
    * it to the DOM.
    *
    ...
    * @throws Exception
    */
   public void generateChildDocument(Document doc,Element currentElement)
                                     throws Exception {
      String strElementName = null;
      log.debug("start generateChildDocument");

      // Okay Normally the Schema Builder objects create unique names to their
      // elements concatenating digits to the end of their names.  These 
      // digits now need to be taken off.

      if(Character.isDigit(this.elementName.
                           charAt(this.elementName.length()-1)))
         this.elementName = this.elementName.
                           substring(0,this.elementName.length()-1);
      if(Character.isDigit(this.elementName.
                           charAt(this.elementName.length()-1)))
         this.elementName = this.elementName.
                           substring(0,this.elementName.length()-1);
      ...
      // Check if their are any more child objects and build their structure 
      // as well by recalling this same method.
      if(childElements.size() > 0) {
         for(int i = 0; i < childElements.size();i++) {
            SchemaBuilder sb = (SchemaBuilder)childElements.get(i);
      ...
      log.debug("end generateChildDocument");
   }
}