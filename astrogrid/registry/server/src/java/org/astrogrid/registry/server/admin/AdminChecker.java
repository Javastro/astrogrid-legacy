...
public class AdminChecker extends DefaultChecker implements IChecker {
    ...
    private static final String ADMIN_WS_NAMESPACE = "http://www.ivoa.net/wsdl";    
    ...
    public boolean checkValidWSContract(Document resourcesNode, String desiredWSInterfaceName) throws CheckerException {
        ...
        if(!resourcesNode.getDocumentElement().getNamespaceURI().equals(ADMIN_WS_NAMESPACE)) {
            throw new CheckerException("Soap Interface NameSpace did not match expected: " + ADMIN_WS_NAMESPACE + " but got: " + resourcesNode.getDocumentElement().getNamespaceURI());            
        }
        return true;
    }
