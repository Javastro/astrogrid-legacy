...
#
# $Id: openjms.sh,v 1.1 2003/07/25 12:54:53 gps Exp $
# -----------------------------------------------------------------------------
...
CLASSPATH="$CLASSPATH":"$OPENJMS_HOME"/lib/openjms-0.7.5-rc1.jar
...
if [ "$1" = "run" ]; then
  ...
  exec "$_RUNJAVA" $JAVA_OPTS -Dopenjms.home="$OPENJMS_HOME" \
      -classpath "$CLASSPATH" \
      -Djava.security.manager -Djava.security.policy="$POLICY_FILE" \
      org.exolab.jms.administration.AdminMgr "$@" -stopServer
  ...
  exec "$_RUNJAVA" $JAVA_OPTS -Dopenjms.home="$OPENJMS_HOME" \
      -classpath "$CLASSPATH" \
      -Djava.security.manager -Djava.security.policy="$POLICY_FILE" \
      org.exolab.jms.administration.AdminMgr "$@"
