...
public class QueryParser3_0
 {
	...
	public static String parseQuery (String query) throws ClassNotFoundException {
		...
		if (xmlResponse.indexOf("ERROR4") == -1){
			//queryResponse = returnRecordKeyPairs(xqlToXML(xqlResponse, searchElements));
			queryResponse = returnRecordKeyPairs(xmlResponse);	
		}
		else if (xmlResponse.indexOf("ERROR5") != -1){
			queryResponse = xmlResponse;	
		}
