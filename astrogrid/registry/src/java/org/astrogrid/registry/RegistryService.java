...
/** The RegistryHandler class builds an xml document and then parses
    the document into a tree.
    Different lists of element nodes have been created for curation metadata,
    serviceType metadata, contact metadata, subjectList metadata,
    and sibling metadata elements of the query element.
    Metadata for services matching either a specific query value or
    all query values is returned for five registry use cases:
 ...
 **/  
  ...
  public String generateResponse( RegistryHandler dom, String queryElement,
                            String queryElementValue, String returnMetadata){
    ...
    NodeList parameterNL =
           parameterDocElement.getElementsByTagName("registryFilename");
...
/** For all nodes in the queryElement nodeList, determine basic, curation,
    service metadata, and service metadataFormat
    metadata.
 **/
                ...
                if(children.item(k).hasChildNodes()) {
                  ...
                  if (childrenOfchildren != null) {
                    ...
                    if(returnMetadata.equals("basic")) {
                      response = response + "<basicMetadata>" + basicMetadataResponse + "</basicMetadata>";
                    }
                    else if(returnMetadata.equals("curation")) {
                      response = response +  "<basicMetadata>" +basicMetadataResponse + "</basicMetadata>"                                 + "<curationMetadata>" + curationMetadataResponse + "</curationMetadata>";
                    }
                    else if(returnMetadata.equals("metadataFormat")) {
                      response = response + "<basicMetadata>" + basicMetadataResponse + "</basicMetadata>"                                    + "<metadataFormat>" + metadataFormatResponse + "</metadataFormat>";
                    }
...
*/
    ...
    String methodName = query.substring((query.lastIndexOf("<method>")+8), (query.indexOf("</method>")));
    String queryElement = query.substring((query.lastIndexOf("<queryElement>")+14), (query.indexOf("</queryElement>")));
    String queryElementValue = query.substring((query.lastIndexOf("<queryValue>")+12), (query.indexOf("</queryValue>")));
    String response = "";
