#!/bin/bash
#Equivalent of maven-build-new.sh
#but customised for building the root site docs
#This is necessary because we don't want to check out
#everything under astrogrid, simply the files and folders
...
DATE=`date`
BUILD_HOME=/home/maven/build/$TAG_TO_BUILD
...
ASTROGRID_VERSION=$TAG_TO_BUILD
...
echo "Build Log for $PROJECT_NAME $ASTROGRID_VERSION on $DATE" >> $LOG_FILE 2>&1
echo "====================================" >> $LOG_FILE 2>&1
...
cvs  -d  $CVSROOT co -l -A -r $TAG_TO_BUILD astrogrid>> $LOG_FILE 2>&1
#but we'll have the xdocs though
rm -r astrogrid/xdocs >> $LOG_FILE 2>&1
cvs -d $CVSROOT co -A -r $TAG_TO_BUILD astrogrid/xdocs >> $LOG_FILE 2>&1
#And the base project
rm -r astrogrid/maven-base >> $LOG_FILE 2>&1
cvs -d $CVSROOT co -A -r $TAG_TO_BUILD astrogrid/maven-base >> $LOG_FILE 2>&1
