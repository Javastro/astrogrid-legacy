...
if maven -Dastrogrid.iteration=snapshot astrogrid-deploy-site >> $LOG_FILE 2>&1 
then
   echo "*** SUCCESS ***\n" >> $LOG_FILE
else
   echo "*** FAILURE ***\n" >> $LOG_FILE
...
if maven -Dastrogrid.iteration=snapshot astrogrid-deploy-snapshot >> $LOG_FILE 2>&1
then
