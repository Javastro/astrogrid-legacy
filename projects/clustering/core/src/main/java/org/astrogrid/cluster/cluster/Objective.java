...
public class Objective {
    ...
    public static result objectiveSpherical(Object cvk, Vector q, Vector mu, Matrix data_er, Matrix S){
        ...
        if(cvk instanceof Double){
            ...
            for (int n = 0; n < ndata; n++){
                f = f +q.get(n)*(-0.5*sum(log(add(cv,S.sliceRow(n))))-0.5*sum(divide(pow(sub(data_er.sliceRow(n),mu),2.0),(add(cv,
                        S.sliceRow(n))))));
            }
            f = -f;
    ...
    public result objectiveFull(Object cvkm, Vector q, Vector mu, Matrix data_er, Matrix S){
        ...
        if (cvkm instanceof Matrix) {
            ...
            double f = 0.0;
            for (int n = 0; n < ndata; n++){
                f=f+q.get(n)*(-0.5*log(det(add(cv, diag(S.sliceRow(n)))))-0.5*multAt(sub(data_er.sliceRow(n),mu),mult(inv(add(cv, 
                        diag(S.sliceRow(n)))),(sub(data_er.sliceRow(n),mu)))).asScalar());
            }
            f = -f;
            // calculate the gradient
