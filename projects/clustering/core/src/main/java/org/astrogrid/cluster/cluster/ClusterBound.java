/*
 * $Id: ClusterBound.java,v 1.1 2009/09/22 07:04:16 pah Exp $
 * 
	            ...
	            switch(noise) {
	                ...
	                case 'spherical'
	                    ...
	                    aux2(n,k)=-ndim_nr/2*log(2*pi)-ndim_nr*log(covk)/2 ...
	                        -0.5*(data_nr(n,:)-gmu_nr(k,:))*(data_nr(n,:)...
	                        -gmu_nr(k,:))'/covk;
	                case 'full'
	                    aux2(n,k)=-ndim_nr/2*log(2*pi)-log(det(squeeze(gcv_nr(k,:,:))))/2 ...
	                        -0.5*((data_nr(n,:)-gmu_nr(k,:))*...
	...
	//     [ndata ndim_int] = size(int_data);
	    for t=1 : ndata
	        for i = 1 : K
	            sgm = ip(i,:);
	            datai = data_int(t,:);
	            aux5(t,i)=sum(datai.*log(sgm)-sgm)-sum(sum(log(1:datai)));
	        end
	    end
	end
